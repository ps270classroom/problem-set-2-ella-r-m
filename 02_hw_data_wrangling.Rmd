...
\## Answer 1 There are 103 candidates in the dataset.
...
candidates_data <- data.frame(
  name = c("RAUNER", "PRITZKER", "RAOUL", "HAROLD", "WHITE", "HELLAND", "FRERICHS", "SALAZAR", "WEISER", "HILL"),
  incumbency_status = c("Incumbent", "Challenger", "Open Seat", "Open Seat", "Incumbent", "Challenger", "Incumbent", "Open Seat", "Open Seat", "Open Seat")
)

print(candidates_data)

incumbency_table <- candidates_data |> 
  group_by(incumbency_status) |> 
  summarize(count = n(), .groups = "drop") |>
  arrange(incumbency_status)
  
print(incumbency_table)

knitr::kable(incumbency_table, caption = "Total number of candidates in each type of incumbency status")        
...
The type of candidate that sponsored the most promoting ads on average was Democrats running for open seats.
...
candidates_data <- data.frame(
  party = c("Republican", "Democrat", "Democrat", "Republican", "Democrat", "Republican", "Democrat", "Democrat", "Democrat", "Republican"),
  incumbency_status = c("Incumbent", "Challenger", "Open Seat", "Open Seat", "Incumbent", "Challenger", "Incumbent", "Open Seat", "Open Seat", "Open Seat"),
  race_type = c("Governer", "Governer", "US House", "US Senate", "Other", "Other", "Other", "Other", "Other", "Other"),
ad_tone_promote = c(0.413, 0.021, 0.0583, 0.0309, 0.0367, 0.169, 0.0848, 0.0171, 0.0151, 0.0180)
)


filtered_data <- candidates_data |> 
...
party_incumbent_promote <-filtered_data |> 
  ...
  knitr::kable(party_incumbent_promote,
               caption = "Average Party and Incumbancy as seen in Ad Tone Promote",
               digits = 3,
               col.names = c("Party", "Incumbancy","Promote Values"))      
...
The shape of the histogram is skewed right. Most of the House candidates did not have more than 10 million ad impressions on facebook.
...
plot_q3 <- ggplot(data = us_house_ads,
       mapping = aes(x= impressions_millions)) + 
  geom_histogram() + 
  labs(title = "Total Facebook ad Impressions for all the US House Races Shown by a Histogram",
...
top_15_us_house_ads <- fb_ads |> 
  filter(office == "US House") |>
  slice_max(impressions_millions, n = 15)



top_15_us_house_ads <- top_15_us_house_ads |>
  mutate(cand_name_last = reorder(cand_name_last, impressions_millions))

fb_top_plot <- ggplot(top_15_us_house_ads, aes(x = impressions_millions, 
                                              y = cand_name_last)) +
  

geom_bar(stat = "identity") + labs(title = "Total Facebook ad Impressions for all the US House Races Shown by a Histogram",
       x = "Impressions (in millions)",
       y = "Candidate Last Names",
       fill = "party") + 
       scale_fill_manual(values = c("R" = "orange", "D" = "purple")) + theme_minimal()
  
print(top_15_us_house_ads)
print(fb_top_plot)
